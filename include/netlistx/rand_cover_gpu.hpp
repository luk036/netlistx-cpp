...
namespace detail {
    ...
    auto make_flat_edges(const Graph& grph, int num_edges)
        -> std::vector<int> {
    ...
    auto make_flat_weights(const Graph& /*grph*/, int num_vertices,
                           const WeightMap& weight) -> std::vector<float> {
        ...
        for (int i = 0; i < num_edges; ++i) {
            ...
            if ((cover[u_word] & u_bit) == 0 && (cover[v_word] & v_bit) == 0) {
                ...
                const double rand_val =
                    static_cast<double>(seed) / 2147483648.0;
    ...
    auto extract_cover_set(const std::vector<unsigned int>& cover,
                           const std::vector<Node>& vertices, int /*num_words*/)
        -> std::vector<Node> {
...
template <typename Graph, typename WeightMap>
auto rand_vertex_cover_gpu(const Graph& grph, const WeightMap& weight,
                           int num_vertices, int num_edges,
                           unsigned int seed = 42u, int num_trials = 1024)
    ...
    run_gpu_trials_raw(edges_flat.data(), num_edges,
                       weights_flat.data(), num_vertices,
                       seeds.data(), num_trials, num_words,
                       covers_host.data(), costs_host.data());
...
auto rand_vertex_cover_gpu(const Graph& grph, const WeightMap& weight,
    ...
    for (int t = 0; t < num_trials; ++t) {
        ...
        detail::pitt_trial_cpu(edges_flat, num_edges, weights_flat, num_vertices,
                               cover, num_words, seeds[t]);
