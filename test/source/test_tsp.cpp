...
TEST_CASE("Christofides TSP — n=5 manhattan")
{
    ...
    // Verify L1 distance between two specific points
    // pts[0]=(0,0), pts[1]=(1,0) → L1 = |1|+|0| = 1.0
    CHECK_EQ(weight(0, 1), 1.0);
    // pts[2]=(0.5,1.0), pts[4]=(0.0,1.0) → L1 = |0.5|+|0| = 0.5
...
TEST_CASE("Combined solver — n=15 manhattan improvement")
{
    ...
    // 2-Opt should never make the Manhattan tour worse
    CHECK_LE(dist_combined, dist_christo + 1.0e-10);
}
